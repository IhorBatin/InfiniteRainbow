...
import com.example.infiniterainbow.domain.usecase.ToggleFavoriteColorUseCase
...
fun InfiniteRainbowApp() {
    ...
    val rainbowViewModel: RainbowViewModel = viewModel(
        factory = remember {
            val dataSource = FavoritesDataSource(context.dataStore)
            val repository = ColorRepositoryImpl(dataSource)
            RainbowViewModel.provideFactory(
        ...
        bottomBar = {
            ...
            BottomNavigation(
                ...
                BottomNavigationItem(
                    ...
                    label = { Text("Explore") },
                    selected = currentDestination?.hierarchy?.any { it.route == "color_list" } == true,
                    onClick = {
